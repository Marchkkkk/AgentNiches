# PROMPTS — lotus_terminal (ЕТАЛОН ніші)

Skeleton: three nested reveals. Narrator: first-person solo analyst, honest skeptic.
Ritual: callback word ("hold this word" planted in hook → returns at climax).
Analogies: 2-3 (core of style). "not A but B" formula: ALLOWED (his signature).
Punchy fragments: allowed. CTA: aggressive comments + subscribe.
Lexicon OWNS: "constraint became the catalyst".
This is the niche reference voice; the other five channels diverge FROM here.

(Canonical copy lives in ../prompts.md — kept verbatim here so each channel has
its own paste-ready file. If the etalon changes, update both.)
